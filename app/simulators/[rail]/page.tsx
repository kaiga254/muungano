...
export default function RailSimulatorPage() {
	...
	const postTransaction = async () => {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(payload.error ?? "Failed to post simulator transaction.");
			}
				...
				<Card className="border-border/70 bg-card/95">
					...
					<CardContent className="space-y-3">
						<div className="flex flex-wrap gap-2">
							{rails.map((entry) => (
								<Link key={entry} href={`/simulators/${entry}`}>
									<Button variant={rail === entry ? "secondary" : "outline"} size="sm">
						...
						<Link href="/simulators">
							<Button variant="ghost" size="sm">← Back to simulator hub</Button>
						</Link>
					</CardContent>
				</Card>
					...
					<CardContent>
						{loadingAccounts ? (
							...
							<p className="text-sm text-muted-foreground">
								No accounts yet. Run payroll for employees to auto-create {rail} sub-accounts.
							</p>
						) : (
									...
									<li key={account.id}>
										...
										>
											<div className="text-left">
												...
												<div className="text-xs text-muted-foreground">{account.accountRef}</div>
											</div>
											<div className="text-right text-sm">
												<div className="font-semibold">{money(account.currentBalance, account.currency)}</div>
											</div>
										</Button>
									</li>
								))}
						...
						{selectedAccount ? (
							<>
								<div className="rounded-lg border border-border/70 bg-muted/30 p-3 text-sm">
									<strong>{selectedAccount.employeeName}</strong> · {money(selectedAccount.currentBalance, selectedAccount.currency)}
								</div>
								...
								<Button onClick={() => void postTransaction()} disabled={submitting}>
									{submitting ? "Posting…" : "Post transaction"}
								</Button>
							</>
						) : (
							<p className="text-sm text-muted-foreground">Select a sub-account first.</p>
						)}
								...
								<TableBody>
									{transactions.map((transaction) => (
										<TableRow key={transaction.id}>
											<TableCell>
												<Badge variant={transaction.direction === "credit" ? "success" : "outline"}>
													{transaction.direction.toUpperCase()}
												</Badge>
											</TableCell>
											<TableCell>{money(transaction.amount, transaction.currency)}</TableCell>
											<TableCell>{money(transaction.balanceAfter, transaction.currency)}</TableCell>
											<TableCell className="max-w-[200px] truncate text-xs text-muted-foreground">{transaction.reference}</TableCell>
											<TableCell className="text-xs text-muted-foreground">{new Date(transaction.createdAt).toLocaleString()}</TableCell>
										</TableRow>
									))}
								</TableBody>
							</Table>
