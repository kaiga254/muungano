	...
	const handleSubmit = async (e: FormEvent) => {
		...
		try {
			...
			const res = await fetch("/api/deposits", {
				...
				body: JSON.stringify({ walletId: walletId || wallets[0]?.id, amount: amountMinor, currency, method }),
			});
			...
			<Card className="border-border/70 bg-card/95">
				<CardContent className="p-5 space-y-4">
					...
					<p className="text-sm text-muted-foreground">
						Your deposit of <strong>{currency} {amountMajor}</strong> via <strong>{method}</strong> is pending.
					...
					<div className="rounded-lg border border-border/60 bg-muted/30 p-4 space-y-2 text-sm">
						{Object.entries(result.instructions).map(([key, val]) => (
							<div key={key} className="flex justify-between gap-4">
								...
								<span className="font-medium font-mono text-right break-all">{String(val)}</span>
							</div>
						))}
					</div>
					<p className="text-xs text-muted-foreground">
						The simulator will automatically confirm your deposit within seconds.
					</p>
					<Button
						variant="outline"
						onClick={() => { setResult(null); setAmountMajor(""); }}
					>
